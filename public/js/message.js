        ...
        if(receiver_list.length === 0){
          ...
          receiver_list.forEach((user, index) => {
            ...
            if(index === 0){
              ...
              user_item = $(`<div class="user_active user" data-receiverID="${user.receiver_id}"><h3 class="user"><i class="fas fa-users"></i><p>${user.user_name}</p></h3><div class="notification hide"></div></div>`);
              ...
              socket.emit("readAllMessage", {owner_id, talker_id: activeuser_id, owner_role})
            }else{
              //確認是否有未讀訊息
              if(user.read_status !== null){
                user_item = $(`<div class="user" data-receiverID="${user.receiver_id}"><h3 class="user"><i class="fas fa-users"></i><p>${user.user_name}</p></h3><div class="notification"></div></div>`);
              }else{
                user_item = $(`<div class="user" data-receiverID="${user.receiver_id}"><h3 class="user"><i class="fas fa-users"></i><p>${user.user_name}</p></h3><div class="notification hide"></div></div>`);
              }
...
function changetalker(e){
  ...
  let hasUnread = !($(`div[data-receiverid=${activeuser_id}]`).find('div').hasClass('hide'));
  if(hasUnread == true){
    //更新訊息為已讀
    socket.emit("readAllMessage", {owner_id, talker_id: activeuser_id, owner_role})
    //hide unread notification
    $(`div[data-receiverid=${activeuser_id}]`).find('div').toggleClass( "hide", true );
  }
...
function renderMessage(message_list){
  ...
  message_list.forEach(message => {
    message_con.append(`<div class="message"><p class="meta">${message.name}<span>${moment(parseInt(message.created_at)).format("YYYY-MM-DD HH:mm")}</span></p><p class="text">${message.content}</p></div>`);
  })
...
function sendMessage(){
  ...
  socket.emit('privateMessage', { content, owner_id, owner_role, talker_id: activeuser_id, talker_role, owner_name, created_at });
  pushMessage(owner_name, content, created_at);
}

function pushMessage(user_name, content, created_at){
  $('#message_con').append(`<div class="message"><p class="meta">${user_name}<span>${moment(created_at).format("YYYY-MM-DD HH:mm")}</span></p><p class="text">${content}</p></div>`);
  // Scroll down
  message_outter[0].scrollTop = message_outter[0].scrollHeight;
}
...
socket.on('privateMessage', (messageInfo) => {
  let { sender_name, sender_id, content, created_at, insertMsgID } = messageInfo;
  if(sender_id === activeuser_id){
    ...
    $(`div[data-receiverID=${sender_id}]`).find('div').toggleClass('hide', false);  //將未讀notification顯示出來
  }
})
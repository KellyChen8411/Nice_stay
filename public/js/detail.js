...
async function rederData(){
    ...
    $('#house_config').text(`${houseData.category_name}-${houseData.people_count}位.${houseData.room_count}間臥室.${houseData.bed_count}張床.${houseData.bathroom_count}間衛浴`);
    //images area
    ...
    if(houseData.refund_type === 1){
        ...
        if(duration>=houseData.refund_duration){
            ...
            clone.find('h4').text(`入住${houseData.refund_duration}天(${refund_duedate})前可免費取消`);
            clone.removeAttr('style');
        }
    }
    ...
    //comment area
    $('#comment_summary>h2').text(`${reviewData[0].house_ave} (${reviewCount}則評價)`)
...
async function getNearbyInfo(e){
    if(e.target.nodeName === "BUTTON"){
        let nearbyLocations = await fetch(`/api/1.0/houses/nearby?lat=${house_lat}&lon=${house_lon}&type=${e.target.dataset.type}`)
        ...
        for (var i = 0; i < nearbyLocations.length; i++) {  
            ...
            var position = new google.maps.LatLng(nearbyLocations[i].lat, nearbyLocations[i].lon);   
